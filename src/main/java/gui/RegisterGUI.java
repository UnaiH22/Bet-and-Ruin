		...
		this.buttonRegister.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				if(password.equals(cPassword)) {
					try {
						businessLogic.registerUser(username, password, eMail);
					} catch (NullParameterException e1) {
						e1.printStackTrace();
						validText.setText("At least one compulsory area was not filled correctly");
					} catch (UserAlreadyTakenException e1) {
						e1.printStackTrace();
						validText.setText("User already taken");
					} catch (NotValidEmailException e1) {
						e1.printStackTrace();
						validText.setText("Invalid eMail.");
					} catch (AlreadyTakenEMailException e1) {
						e1.printStackTrace();
						validText.setText("eMail already taken.");
